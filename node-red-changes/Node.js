...
function Node(n) {
    var fs = require('fs');
    //NOTE. this takes the token once the node is deployed
    this.token = fs.readFileSync('/tmp/token.txt').toString().replace('\n','').replace('\r','');
    this.id = n.id;
    this.type = n.type;
    this.z = n.z;
...
Node.prototype.receive = function(msg) {
    ...
    try {
       //NOTE: this takes the token from the file every time (i.e. logged in user)
        msg.token = require('fs').readFileSync('/tmp/token.txt').toString().replace('\n','').replace('\r','');
        //NOTE: This gets the token only from the moment when it was deployed
